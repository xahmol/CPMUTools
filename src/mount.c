...
unsigned char DirH = 18; // Number of entries per column
unsigned char DirC = 2;  // Number of columns
unsigned char DirW = 25; // Width of one column
...
void Readdir()
{
    ...
    while (uii_isdataavailable())
    {
        ...
        // print progress bar
        if ((count >> 2) >= DirW * DirC)
        {
            xpos = 0;
            count = 0;
            ClearArea(0, 24, DirW * DirC, 1);
        }
        else
        {
        ...
        if (!presenttype && datalength > 4)
        {
                 ...
                 uii_data[datalength - 3] == 'g' || uii_data[datalength - 3] == 'G'))
            {
                ...
                // Check for DNP
                if ((uii_data[datalength - 2] == 'N' || uii_data[datalength - 2] == 'n') &&
                    (uii_data[datalength - 1] == 'P' || uii_data[datalength - 1] == 'p'))
                {
                    presenttype = 5;
                }
            }
...
void DrawIDandPath()
{
    ...
    if (length > DirW * DirC)
    {
        strcpy(buffer, uii_data + length - (DirW * DirC));
    }
    else
    {
...
void PrintDirEntry(struct DirElement *present, unsigned char printpos)
{
    ...
    unsigned char xpos = (printpos > DirH - 1) ? DirW : 0;
    unsigned char ypos = (printpos > DirH - 1) ? printpos - 13 : 5 + printpos;
...
void DrawMenu()
{
    ...
    printstrvdc(51, ypos++, colorText, "A-E   : Select target");
    ...
    printstrvdc(51, ypos++, colorText, "T / L : Go Top/Last");
...
void MountImage(ushort target, char *image)
{
    ...
    ushort ult_drive = (validdrive[target - 1] && validdrive[target - 1] < 3);

    if (!validdrive[target - 1])
    {
        return;
    }
    ...
    if (!CheckUCIStatus())
    {
        ...
        printstrvdc(0, 23, colorSuccess, "Success!");
        sprintf(buffer, "%s mounted on %c, ID %2d", image, target + 'A' - 1, target + 7);
        printstrvdc(9, 23, colorText, buffer);
...
void saveREU()
{
    ...
    if (textInput(0, 24, reuname, 16) == -1)
    {
        ClearArea(0, 23, 80, 2);
        return;
    }
    ...
    do
    {
        ...
        case CURU_DOWN:
            ...
            if (presentdirelement && presentdirelement->next)
            {
                ...
                presentdirelement = present;
                presentdir.position++;
                if (presentdir.position > (DirH * DirC) - 1)
                {
                    // Next page
                    presentdir.firstprint = present;
                    presentdir.position = 0;
        ...
        case CURU_UP:
            ...
            if (presentdirelement && presentdirelement->prev)
            {
                ...
                {
                    // Previous page
                    // Go back positions for one page - 1
                    for (element = 0; element < (DirH * DirC) - 1; element++)
                    {
                        if (present->prev)
                        {
        ...
        case CURU_RIGHT:
            // Go right
            if (presentdir.position < DirH && DirC == 2)
            {
                ...
                for (element = 0; element < DirH; element++)
                {
                    if (present->next)
                    {
                        present = present->next;
        ...
        case 'p':
            if (presentdirelement->next)
            {
                ...
                presentdir.position = 0;
                // Go forward one page in positions
                for (element = 0; element < (DirH * DirC); element++)
                {
                    if (present->next)
                    {
                        present = present->next;

...
void delay(ushort D)
{
  ...
  while ((inp(cia2 + ciaIntCtrl) & 0x02) == 0)
    ; /* wait for count down */
}
...
void WriteConfigfile(unsigned char vdcmode)
{
  ...
  if (!file)
  {
    ErrorMessage("opeming for write", 1);
  }

  // Write to file
  if (!fwrite(&config, sizeof(config), 1, file))
  {
    fclose(file);
    ErrorMessage("writing", 1);
  }
...
void ReadConfigfile(unsigned char vdcmode)
{
  ...
  {
    ClearArea(0, 24, 80, 1);
    WriteConfigfile(vdcmode);
    return;
  }

  // Reading file
  if (!fread(&config, sizeof(config), 1, file))
  {
    fclose(file);
    ErrorMessage("reading", 1);
  }
